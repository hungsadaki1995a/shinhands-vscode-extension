...
export class ComposedLoader extends Loader {
  ...
  get flattenLocaleTree(): FlattenLocaleTree {
    if (!this._isFlattenLocaleTreeDirty) return this._flattenLocaleTree;
  ...
  getValueByKey(
    ...
    for (const loader of this.loadersReversed) {
      const value = loader.getValueByKey(
        keypath,
        locale,
        maxLength,
        stringifySpace
  ...
  async write(
    pendings: PendingWrite | PendingWrite[],
    triggerFullfilled = true
  ) {
    if (!Array.isArray(pendings)) pendings = [pendings];

    if (Config.keepFulfilled && triggerFullfilled) {
      pendings = [
        ...((await FulfillAllMissingKeys(
          false,
          uniq(pendings.map(i => i.keypath))
        )) || []),
        ...pendings
      ];
    }
    ...
    const distrubtedPendings: PendingWrite[][] = new Array(
      this.loadersReversed.length
    )
      .fill(null)
      .map(() => []);
    ...
    for (const pending of pendings) {
      ...
      if (!handled) Log.info(`💥 Unhandled write ${JSON.stringify(pending)}`);
    }
