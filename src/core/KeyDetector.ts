...
export class KeyDetector {
  ...
  static getKeyAndRange(
    document: TextDocument,
    position: Position,
    dotEnding?: boolean
  ) {
    const { range, key } =
      KeyDetector.getKeyRange(document, position, dotEnding) || {};
    if (!range || !key) return;
  ...
  static getKeys(
    ...
    if (typeof document !== "string") {
      ...
      scopes =
        scopes ||
        Global.enabledFrameworks.flatMap(f => f.getScopeRange(document) || []);
    } else {
  ...
  static getUsages(
    ...
    if (localeFile) {
      type = "locale";
      const parser = Global.enabledParsers.find(p =>
        p.annotationLanguageIds.includes(document.languageId)
      );
      if (!parser) return;
